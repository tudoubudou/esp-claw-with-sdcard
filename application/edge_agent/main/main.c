...
void app_main(void)
{
    ...
    esp_err_t fs_err = app_fs_init();
    if (fs_err != ESP_OK) {
        ESP_LOGW(TAG, "app_fs_init failed: %s (continuing with limited storage)", esp_err_to_name(fs_err));
    }
    ...
    if (claw_err != ESP_OK) {
        ESP_LOGW(TAG, "app_claw_start failed: %s (continuing without agent)", esp_err_to_name(claw_err));
    }
#if CONFIG_APP_CLAW_CAP_IM_LOCAL
    esp_err_t webim_err = http_server_webim_bind_im();
    if (webim_err != ESP_OK) {
        ESP_LOGW(TAG, "webim_bind failed: %s (continuing)", esp_err_to_name(webim_err));
    }
#endif
