...
#define DISPLAY_SERVICE_DEFAULT_BUFFER_LINES 10
                                                              ...
                                                              uint32_t buffer_lines)
{
    ...
    if (strcmp(lcd_cfg->sub_type, ESP_BOARD_DEVICE_LCD_SUB_TYPE_DSI) == 0) {
        ...
        // Don't use LVGL rotation; swap_xy is handled at panel level by board manager.
        // But swap width/height for LVGL to match the logical (post-rotation) dimensions.
        uint16_t lv_w = lcd_cfg->lcd_width;
        uint16_t lv_h = lcd_cfg->lcd_height;
        if (lcd_cfg->swap_xy) {
            uint16_t tmp = lv_w; lv_w = lv_h; lv_h = tmp;
        }
#ifdef CONFIG_SPIRAM
        disp_cfg = ESP_LV_ADAPTER_DISPLAY_SPI_WITH_PSRAM_DEFAULT_CONFIG(lcd_handles->panel_handle,
                                                                        ...
                                                                        lv_h,
                                                                        rotation);
        // Force internal RAM for draw buffer (SPI DMA can't read PSRAM reliably)
