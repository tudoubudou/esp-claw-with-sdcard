...
{
    // On ESP32-S3 with Octal PSRAM, esp_console_new_repl_uart() fails and its
    // partial VFS initialization causes infinite recursion through the UART
    // logging path (esp_log -> vprintf -> fflush -> uart_write_bytes -> esp_log),
    // leading to stack overflow and xQueueSemaphoreTake assertion. The web UI
    // provides equivalent functionality, so the serial REPL is not needed.
    (void)0;
    return ESP_OK;
}
