...
static void app_time_sync_success(bool had_valid_time, void *ctx)
{
    (void)ctx;
    (void)had_valid_time;
    // NOTE: cap_scheduler_handle_time_sync() is intentionally not called here.
    // The time sync service task runs on a PSRAM-preferred stack, and the
    // scheduler rebase path triggers flash/NVS writes which assert on
    // Octal PSRAM stacks (spi_flash_disable_interrupts_caches_and_other_cpu).
    // The scheduler still loads and runs; it just won't auto-rebase on first sync.
}
#endif
